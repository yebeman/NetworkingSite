	 ...
	 <div style='margin-top:70px;margin-left:-100px; ' id='sticky_navigation_wrapper'>
        <div id='sticky_navigation'>
            <div class='demo_container'>
                <ul>
                    <li><a  href='http://localhost/yc/profile' href='#' >About Me</a>
                    </li>
...
// taking albums and uid
$sql = mysql_query("SELECT * FROM `members` WHERE  emailAddress='$emailAddress'"); 
$row = mysql_fetch_array($sql);

$uid=$row['id'];
$album=$row['albums'];
$album = explode("|s|", $album);

$photo= '<table border="0"  cellpadding="2" align="center"><tr>';

for($x=0;$x<count($album);$x++){
	
		if($x%3==0&&$x!=0)
		$photo.="</tr><tr>";
  $sql = mysql_query("SELECT * FROM `photo` WHERE  uid='$uid' and albumId='$album[$x]'"); 
	$row = mysql_fetch_array($sql);

	$imageSrc =$row['http'];
	$noPhotos =$row['noPhotos'];
	$imageLikes =$row['likes'];
	
	$photo.= '<td><div class="viewport no-margin"><a href="full.php?album='.$album[$x].'"><p class="NOP">'.$noPhotos.' photos</p> <span class="dark-background">'.$album[$x].'<em>'.$imageLikes.' likes</em></span><img src='.$imageSrc.' /></a> </div></td>';
}
$photo.='</tr></table></div></div>';	
	$html.=$photo;
		// bar for the online box
   $html.="<div  id='onlineF' style=' top:35px;position: fixed; float: right; right: 2%;'>
...
$(document).click(function(e) { // when any click is received
    if (
	 ...
	 $('.viewport').mouseenter(function(e) {
            $(this).children('a').children('img').animate({ height: '220', left: '0', top: '0', width: '290'}, 100);
            $(this).children('a').children('span').fadeIn(200);
        }).mouseleave(function(e) {
            $(this).children('a').children('img').animate({ height: '250', left: '-20', top: '-20', width: '320'}, 100);
