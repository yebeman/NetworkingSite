...
<link rel="stylesheet" type="text/css" href="css/photoGridS.css" />
<link href="css/demoStyles.css?1.0" rel=stylesheet type="text/css" />
<link href="css/styleMsg.css" rel=stylesheet type="text/css" />
<link rel="stylesheet" type="text/css" href="css/logoutMenu.css" />

</head>
<body class="mainmenu-push">
      <?php
						...
						if (isset ( $_COOKIE ['em'] ) && isset ( $_COOKIE ['sub'] )) {
							...
							// if not then "no good"
							if ($login_check <= 0) {
								header ( "Location: http://localhost/yca/index.php" );
							} else {
...
";
						$html .= "<div align='center' style='display: block;'>";
	 ...
	 <div style='margin-top:70px;margin-left:-100px; ' id='sticky_navigation_wrapper'>
        <div id='sticky_navigation'>
            <div class='demo_container'>
                <ul>
                     <li><a  href='http://localhost/yca/profile' >About Me</a>
                    </li>
                     <li><a class='selected' href='http://localhost/yca/photo'>Photo</a>
                    </li>
                    <li><a href='http://localhost/yca/friend'>Friends</a>
                    </li>
                    <li><a href='http://localhost/yca/myapps'>Apps</a>
                    </li>
                </ul>
            </div>
        </div>
...
<div  style=' display: block; width:900px;background-color:#F1F1F1; padding:20px 0px 10px 0px; margin-left:auto; margin-right:auto;  margin-top:75px;'>";
						...
						$emailAddress = "yebe@gmail.com";
						// no need but for the moment
						
						// type in here
						
						$html .= '<div style="display: inline-block;">
               <ul class="parrent" id="grid">';
						
						$album = $_REQUEST ['album'];
						// "Work Hard";//$_POST['emailAddress'];
						$uid = 12;
						// $_POST['emailAddress'];
						
						$sql = 'SELECT * FROM `photo` WHERE  uid="' . $uid . '" and albumId="' . $album . '"';
						
						// Do Search
						$result = $tutorial_db->query ( $sql );
						...
						if (isset ( $result_array )) {
							foreach ( $result_array as $result ) {
								$html .= '<li class="viewport"><a  href="' . $result ['http'] . '" data-lightbox="example-set" desc="' . $result ['desc'] . '" like="' . $result ['likes'] . '" cmt="' . $result ['noCmts'] . '"  view="' . $result ['seen'] . '"><span class="dark-background">' . $result ['desc'] . '</span><img src="' . $result ['http'] . '" > </a> <ul class="child"> <li class="cmts"><a title="comment" href="' . $result ['http'] . '" data-lightbox="0002" desc="' . $result ['desc'] . '" like="' . $result ['likes'] . '" cmt="' . $result ['noCmts'] . '"  view="' . $result ['seen'] . '">' . $result ['noCmts'] . '</a></li>   <li class="likes"><a id="' . $result ['http'] . '" title="Like" href="#" onClick="likefunc(this.id);">' . $result ['likes'] . '</a></li> <li class="views">' . $result ['seen'] . '</li> </ul></li>';
							}
						}
						
						// header
						$html .= "</ul></div></div><header id='topdiv'> <table width='99%' style='position:fixed;margin-top:-5px; min-width:560px; ' cellspacing='0' border='0' cellpadding='0'><tr valign='top'>";
						
						$html .= "<td width='33%'><a id=bottomBtn href='http://localhost/yca/' class=button>yachatt </a></td>";
...
<ul>
 <li id='marquee-author' class='marquee-author'> </li></ul>";
						...
						//
						// Right menu
						$html1 .= "<nav class='mainmenu mainmenu-vertical mainmenu-left' id='mainmenu-s1'> <h3>Your pages</h3>
  " . $pagesI . "
   <button class='multBtn' style='position:absolute;margin-left:210px; bottom:0px;' id='showLeftPush'> Profile</button> </nav>";
						
						// Left menu
						$html1 .= " <nav class='mainmenu mainmenu-vertical mainmenu-right' id='mainmenu-s2'>
			<h3>Newsfeed</h3>
			<a href='#'>News</a>
	...
	<script type="text/javascript" src="js/runMsg.js"></script>
	...
	<script type="text/javascript">
         ...
         $(document).ready(function() {
            ...
            var searchN = $('div.icon');
            // cache the searchN element for speed
            var logoutM = $('dd');
            $(document).click(function(e) {// when any click is received
               if ((searchN[0] != e.target) && // the target element is not the searchN
            ...
            $(".marquee").marquee({
               ...
               aftershow : function($marquee, $li) {
                  ...
                  // hide the author
                  if ($author.length)
                     $("#showRightPush").find("> span").fadeOut(250);
               }
            });
         });
         ...
         ///////////
         var menuLeft = document.getElementById('cbp-spmenu-s1'), menuRight = document.getElementById('cbp-spmenu-s2'), showRightPush = document.getElementById('showRightPush'), showLeftPush = document.getElementById('showLeftPush'), body = document.body;
	...
	<script type="text/javascript" src="js/readPhoto.js"></script>
